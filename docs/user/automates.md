...
`Expenses --> Groceries` folder. Incoming documents are those
...
![list of document automate instances](../img/user-manual/automates/01-automates-v2.png)
...
!!! danger
    ...
    Automation processes is triggered every time when OCR for
    certain page completes. OCR page is sent to automation module and
...
![docs matching](../img/user-manual/automates/document-matching-keywords-delimited-by-space-v2.png)
...
`from email account` end up in your `Inbox` folder.
...
![logs ui](../img/user-manual/automates/02-ui-logs-v2.png)
...
!!! note
  ...
  UI Logs are very convenient to see the **extracted text** from the document.
...
![manually run automates](../img/user-manual/automates/manually-run-automates-v2.png)
...
move Schnell receipts to `Expenses --> Groceries` folder and
will tag them with `groceries`, `receipt`, `schnell` tags.
For this purpose, following Automate was created:

![automate and two receipts](../img/user-manual/automates/automate-and-two-receipts.png)

Notice that `Match` field has as value one single lowercase keyword "schnell".
...
!!! note
  ...
  Automates are triggered for all `incoming documents`, even if they are
  manually uploaded to `Inbox` folder. However, if you cut/paste document(s)
  from other folder to `Inbox`
  - automates won't run.
...
![troubleshooting automates](../img/user-manual/automates/troubleshooting-automates-step-2.png)
...
![OCR engine made a mistake](../img/user-manual/automates/ocr-engine-made-a-mistake.png)
...
![automate with two keywords](../img/user-manual/automates/automate-with-two-keywords.png)
...
![corretly applied automates](../img/user-manual/automates/correctly-applied-automates.png)
