...
you need to install so called module [mod_wsg](https://modwsgi.readthedocs.io/en/develop/index.html)
...
Get latest release of `mod_wsgi` from [here](https://github.com/GrahamDumpleton/mod_wsgi/releases). Extract archive:
...
+ Gunicorn` duo. [Gunicorn](https://gunicorn.org) is called application server - it serves WSGI
...
[Gunicorn](https://gunicorn.org/) is not provided in list of dependencies. Thus,
you need to installed in your current virtual python environment:
...
in this [ticket](https://github.com/ciur/papermerge/issues/198) on github.
...
It uses [redis](https://redis.io/) key value database. With redis as broker transport you will
...
!!! important
    ...
    is the one in [project_dir/config/base.py](https://github.com/ciur/papermerge/blob/master/config/settings/base.py)
