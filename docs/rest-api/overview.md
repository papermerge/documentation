...
wide-spread http client is curl.
...
`<server-url>` - this is base http address of your {{ extra.project }}
instance. Base url/address is where your instance was deployed.
Depending where your {{ extra.project }} instance was deployed you may use
...
!!! note
    ...
    When querying for json data types always pay attention to
...
!!! note
   ...
   When quering for json data types always pay attention to
...
In {{ extra.project }} one folder can contain other folders and documents. It is de
...
files). In this regard {{ extra.project }} behaves like a common `file manager`.
...
!!! important
    ...
    Every folder and document you create in
    {{ extra.project }} requires non empty folder parent id field.


## Special Folders

Every user in {{ extra.project }} has two special folders:
...
!!! note
    ...
    You can create "My Documents" folder directly in user's ``.inbox``. However
...
folder id is actually an `uuid` number, however for sake of simplicity,
we abbreviate it and remove all irrelevant fields.

!!! important

    For creating folder and documents you always need parent folder
...
{{ extra.project }} does not impose folder title uniqueness.
...
!!! important
    ...
    When content-type  is `application/vnd.api+json`, always
...
!!! note
    ...
    Even though you can use both endpoints i.e. `POST /api/nodes/`
...
!!! important
     ...
     Correct endpoint for listing the content of specific folder is
